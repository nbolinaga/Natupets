<template>
	<div
		class="rounded-xl border-2 border-yellow-400 py-10 px-4 md:p-10 bg-gray-50 text-center overflow-hidden shadow-md">
		<h3
		...
		<ul v-if="opened == true">
			<li
				...
				<p class="mb-4 text-sm">Pedido hecho el {{ pedido.fecha }}</p>
				<button
				...
				<p v-if="pedido.entregado" class="text-yellow-500 text-xs">
					Entregado el {{ pedido.fechaEntrega }}
				</p>
				<p v-if="pedido.pagado" class="text-green-500 text-xs">
					Pagado el {{ pedido.fechaPago }}
				</p>
				<p v-if="pedido.cancelado" class="text-red-500 text-xs">
					Cancelado el {{ pedido.fechaCancelado }}
				</p>
				<p v-if="pedido.marcado" class="text-blue-500 text-xs">Pago confirmado</p>
				<div
